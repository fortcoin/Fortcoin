...
bool CNode::DisconnectOldProtocol(int nVersionRequired, string strLastCommand)
{
    ...
    if (nVersion == 70103) {
      LogPrintf("\nDEBUG: nversion is 70103\n");
    }

    if (nVersion < 70103) {
      LogPrintf("\nDEBUG: nversion is less than 70103\n");
    }

    // Handle old clients trying to connect without the base upgrade specifically
    if (chainActive.Height() >= Params().LAST_POW_BLOCK() && nVersion < nVersionRequired) {
        LogPrintf("%s : peer=%d using obsolete version %i; disconnecting height: %d or here\n", __func__, id, nVersion, chainActive.Height());
        PushMessage("reject", strLastCommand, REJECT_OBSOLETE, strprintf("Version must be %d or greater", ActiveProtocol()));
	return true;
    } else if (chainActive.Height() < Params().LAST_POW_BLOCK() && nVersion < 70103) {
        LogPrintf("%s : peer=%d using obsolete version %i; disconnecting height: %d here\n", __func__, id, nVersion, chainActive.Height());
        PushMessage("reject", strLastCommand, REJECT_OBSOLETE, strprintf("Version must be %d or greater", ActiveProtocol()));
        fDisconnect = true;
	return true;
    }
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (FORT users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "FORT " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. FORT Core is probably already running."), addrBind.ToString());
        else
